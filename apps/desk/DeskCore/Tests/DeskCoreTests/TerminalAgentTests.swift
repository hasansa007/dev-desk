...
/// The terminal commands are the ones that ran on 2026-09-16, byte for byte: a guessed flag is a terminal that
/// opens and fails in front of the developer.
final class TerminalAgentTests: XCTestCase {
    ...
    func testEachCommandStaysInteractiveAndGivesTheFamilyFolderWhereTheToolFencesItsFiles() {
        ...
        XCTAssertEqual(TerminalAgent.gemini.command(prompt: prompt, familyRoot: root),
                       "gemini --include-directories '/Users/me/.claude/skills/dev' -i '\(prompt)'")
        XCTAssertEqual(TerminalAgent.opencode.command(prompt: prompt, familyRoot: root),
                       "opencode --prompt '\(prompt)'")
        XCTAssertEqual(TerminalAgent.antigravity.command(prompt: prompt, familyRoot: root),
    ...
    func testAQuoteInThePromptCannotEndTheShellString() {
        let command = TerminalAgent.opencode.command(prompt: "fix the user's upload", familyRoot: root)
    ...
    func testDetectionAsksForEachExecutableAndOffersOnlyWhatAnswered() async {
        ...
        let found = await TerminalAgent.detect(runner: runner)
