...
| A bare family name — `run`, `prod`, `pre-prod`, `verify`, `docs`, `review`, `trim` | **Confirm the handoff** — see the guard below |
...
- **a family name** — `run`, `prod`, `pre-prod`, `verify`, `docs`, `review`, `trim`, with or without the
