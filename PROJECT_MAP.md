...
| Install | `install.sh` symlinks the clone into each CLI's skills dir as `dev`, and links `scripts/dev.py` as the `dev` command into `~/.local/bin` (or `~/bin`) when one is already on `PATH`. **`apps/desk/install.sh` is a different thing**: it builds Dev Desk Release, ad-hoc signed, into `~/Applications` — the bundle you actually open, which is not the DerivedData one Xcode builds (ADR 0024's context) |
